...
struct __CFRunLoopMode {
    ...
    CFStringRef _name; // model 的名字，例如 @"kCFRunLoopDefaultMode"
    Boolean _stopped;
    char _padding[3];
    CFMutableSetRef _sources0;// Set 集
    CFMutableSetRef _sources1;// Set 集
...
/** 一个线程在运行期内，可能多次进入退出交替序列，每一次交替序列的进入与退出，都会伴随独立的_per_run_data实例变量的创建与销毁
 * 是用来描述当前CFRunLoop的状态的，有三种状态：初始状态，wake，stop三种
 */
...
/** CFRunLoop 的结构
 * @param _pthread 一个RunLoop包含一个线程，与线程是一一对应的关系！
 * @param _currentMode 一个RunLoop有很多 mode，但是只有一个 _currentMode
 * @param _modes 每次只能在一个mode下运行RunLoop，如果需要切换Mode，只能退出 RunLoop，不可能在多个Mode下同时运行；mode只能添加不能删除；
 * @param _commonModes 使用CFRunLoopAddCommonMode()函数将Mode标记为common属性，并保存在 _commonModes；
 * @param _commonModeItems 存放的source,observer,timer等，在每次RunLoop运行的时候都会被同步到具有Common标记的_commonModes里。
 */
struct __CFRunLoop {
    CFRuntimeBase _base;
    pthread_mutex_t _lock;			/* 关于 mode list 的锁 */
    __CFPort _wakeUpPort; //用来被唤醒的端口： 调用 CFRunLoopWakeUp() 函数
    Boolean _unused;    //是否使用
    volatile _per_run_data *_perRunData;              // reset for runs of the run loop
    pthread_t _pthread;
    uint32_t _winthread;
    CFMutableSetRef _commonModes;// Set集，
    CFMutableSetRef _commonModeItems;
    //保存的就是那些需要同步添加到具有Common属性的Mode中的Source/Timer/Observer集合。
    CFRunLoopModeRef _currentMode; //代表当前RunLoop对象正在执行的Mode
    CFMutableSetRef _modes;//是一个Set集合,保存了所有该RunLoop对象可以执行的Mode。
...
保存的是具有Common属性的Mode的名称，前文__CFRunLoopMode的结构体定义中可以看到，每个Mode管理自己的Source/Timer/Observer，而被标记为Common属性的Mode还有一个特性就是当RunLoop对象在执Common属性的Mode时，会自动将_commonModeItems中保存的Source/Observer/Timer同步添加该Mode中，标识Common属性只需要将__CFRunLoopModeRef的_name成员变量的值添加进_commonModes集合中即可。被标记为Common属性的Mode就是前文讲的kCFRunLoopCommonModes模式，可以看出这种模式不是一种真正的模式，仅仅是标识其他模式是否需要同步添加_commonModeItems中的Source/Timer/Observer。
...
/** 根据 modeName 在一个 RunLoop 中找出对应的 Mode
 * @param rl 指定的 RunLoop
 * @param modeName 指定的Mode名字
 * @param create 如果找不到Mode，是否需要创建一个
 */
...
/** 根据指定的线程ID 创建对应的 RunLoop
 * @param pthread_t 线程ID
...
#pragma mark - RunLoop与线程的关系

/** RunLoop与线程的关系
 * 1.线程和RunLoop之间是一对一的关系，其关系保存在一个全局字典 __CFRunLoops 中
 * 2 主线程的 RunLoop 系统默认创建；
 * 3、分线程的 RunLoop 不会自动创建，只有主动获取时才会被创建。
 * 4、在对应线程结束时 RunLoop 会被销毁
 */
...
/** 并发编程—OSAtomic原子操作: 比较和交换
 * 比较 oldValue是否与 theValue指针指向的内存位置的值匹配。
 * @return 如果两个值相等，则将 newValue的值存储到 theValue指向的内存位置；返回 YES
 *         否则,它什么都不做。
 * OSAtomicCompareAndSwapPtrBarrier(type __oldValue, type __newValue, volatile type *__theValue)：
*/

/** 获取指定线程的 Runloop
 * @param pthread_t 指定的线程；t==0 代表主线程
 * @note 该函数主要做了以下几件事：
 *    1、如果字典 __CFRunLoops 为空则初始化一个，并且创建 mainLoop 存储到 __CFRunLoops
 *    2、根据指定的线程ID从字典获取对应的CFRunLoopRef
 *    3、如果没有取到，就根据线程ID创建一个 CFRunLoopRef
 */
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    if (pthread_equal(t, kNilPthreadT)) {
        t = pthread_main_thread_np();//如果是主线程，赋值等于主线程ID
    }
    __CFLock(&loopsLock);//访问 loopsDic 时的自旋锁
    
    //如果字典 __CFRunLoops 为空则初始化一个，并且创建 mainLoop 存储到 __CFRunLoops
    if (!__CFRunLoops) {
        __CFUnlock(&loopsLock);
        // 第一次进入时，初始化全局dict，并先为主线程创建一个 RunLoop。并将mainLoop添加到dict中
        CFMutableDictionaryRef dict = CFDictionaryCreateMutable(kCFAllocatorSystemDefault, 0, NULL, &kCFTypeDictionaryValueCallBacks);
        //传入主线程ID，创建主线程关联的 Runloop
        CFRunLoopRef mainLoop = __CFRunLoopCreate(pthread_main_thread_np());
        //将mainLoop添加到dict中
        CFDictionarySetValue(dict, pthreadPointer(pthread_main_thread_np()), mainLoop);
        //如果 __CFRunLoops 是空指针，则__CFRunLoops=dict，并返回YES,不能将dict释放
        if (!OSAtomicCompareAndSwapPtrBarrier(NULL, dict, (void * volatile *)&__CFRunLoops)) {
            //如果 __CFRunLoops 不为空指针，则 dict 没有必要存在，释放掉
            CFRelease(dict);
        }
        CFRelease(mainLoop);//释放mainLoop
        __CFLock(&loopsLock);
    }
    
    //根据指定的线程ID从字典获取对应的CFRunLoopRef
    CFRunLoopRef loop = (CFRunLoopRef)CFDictionaryGetValue(__CFRunLoops, pthreadPointer(t));
    __CFUnlock(&loopsLock);
    if (!loop) {// 取不到 CFRunLoopRef 就创建一个
    ...
    if (pthread_equal(t, pthread_self())) {
        _CFSetTSD(__CFTSDKeyRunLoop, (void *)loop, NULL);/// 注册一个回调，当线程销毁时，顺便也销毁其对应的 RunLoop。
...
/** 在每个线程退出时调用该函数销毁其对应的 RunLoop
 * 该函数主要做了以下几件事：
 * 1、 解除全局字典__CFRunLoops中RunLoop与线程之间的映射关系
 * 2、移除RunLoop中Set<Mode>集合里所有mode的source
 * 3、移除mode中Set<commonModes>所有的 source
 */
CF_PRIVATE void __CFFinalizeRunLoop(uintptr_t data) {
    ...
    if (rl && CFRunLoopGetMain() != rl) { // protect against cooperative threads
        ...
        for (CFIndex idx = CFArrayGetCount(array); idx--;) {// 移除runLoop中Set<Mode>集合里所有mode的source
...
/** 用于接收指定端口的消息，是实现 Runloop 休眠的核心处理
 * @param timeout 当端口无消息的时候是否休眠，0是立刻返回不休眠，TIMEOUT_INFINITY 代表休眠
 */
static Boolean __CFRunLoopServiceMachPort(mach_port_name_t port, mach_msg_header_t **buffer, size_t buffer_size, mach_port_t *livePort, mach_msg_timeout_t timeout, voucher_mach_msg_state_t *voucherState, voucher_t *voucherCopy) {
    ...
    for (;;) {		/* In that sleep of death what nightmares may come ... */
        ...
        msg->msgh_id = 0;
        //根据timeout参数的值来判断是让runloop 休眠还是继续查询（poll）
        if (TIMEOUT_INFINITY == timeout) { CFRUNLOOP_SLEEP(); } else { CFRUNLOOP_POLL(); }
        //
        /*调用mach_msg()函数接收消息,
         * 如果没有port消息，内核会将线程置于等待状态mach_msg_trap()，Runloop没事做就休眠了
         * 如果没port消息,根据接收消息的结果对livePort进行赋值，
         *      一种是成功获取到消息后，会根据情况赋值为msg->msgh_local_port或者MACH_PORT_NULL，
         *      另一种获取消息超时的情况会赋值为MACH_PORT_NULL。
         */
 ...
 *  @return 返回运行的状态：
 *
 * https://www.jianshu.com/p/37b85491bf91
 */
/* rl, rlm are locked on entrance and exit */
static int32_t __CFRunLoopRun(CFRunLoopRef runLoop, CFRunLoopModeRef theMode, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    } else if (theMode->_stopped) {
        theMode->_stopped = false;
        return kCFRunLoopRunStopped;
    }
    ...
    if (libdispatchQSafe && //在主线程
        (CFRunLoopGetMain() == runLoop) && // runloop是主线程的runloop
        CFSetContainsValue(runLoop->_commonModes, theMode->_name)){ // 该 Mode 是 CommonMode
        //给 mach 端口赋值为主线程收发消息的端口
    ...
    if (theMode->_queue) {
        ...
        modeQueuePort = _dispatch_runloop_root_queue_get_port_4CF(theMode->_queue);
        if (!modeQueuePort) {
    ...
    if (seconds <= 0.0) { // instant timeout
        ...
        timeout_context->rl = (CFRunLoopRef)CFRetain(runLoop);
    ...
    // RunLoopRun() 返回值，默认为0，会在do while中根据情况被修改，当不为0时，runloop退出
    int32_t retVal = 0;
    ...
    do {
...
#if DEPLOYMENT_TARGET_MACOSX || DEPLOYMENT_TARGET_EMBEDDED || DEPLOYMENT_TARGET_EMBEDDED_MINI
        mach_msg_header_t *msg = NULL; // 用于记录唤醒休眠的RunLoop的mach port，休眠前是NULL
        ...
        __CFPortSet waitSet = theMode->_portSet;
        //设置RunLoop为可以被唤醒状态
        __CFRunLoopUnsetIgnoreWakeUps(runLoop);

        // 2. 通知 Observers: RunLoop 即将处理 Timers，并执行回调
        if (theMode->_observerMask & kCFRunLoopBeforeTimers)
            __CFRunLoopDoObservers(runLoop, theMode, kCFRunLoopBeforeTimers);
        
        // 3. 通知 Observers: RunLoop 即将处理 sources，并执行回调
        if (theMode->_observerMask & kCFRunLoopBeforeSources)
            __CFRunLoopDoObservers(runLoop, theMode, kCFRunLoopBeforeSources);
        
        // 处理提交到runloop的blocks
        __CFRunLoopDoBlocks(runLoop, theMode);

        // 4. RunLoop 触发 Source0 (非port) 回调
        Boolean sourceHandledThisLoop = __CFRunLoopDoSources0(runLoop, theMode, stopAfterHandle);
        ...
        if (!poll && (theMode->_observerMask & kCFRunLoopBeforeWaiting)) __CFRunLoopDoObservers(runLoop, theMode, kCFRunLoopBeforeWaiting);
        __CFRunLoopSetSleeping(runLoop);//设置RunLoop为休眠状态（并还没有进入休眠）
         //在这行之后，不在做任何callouts
	...
	// do not do any user callouts after this point (after notifying of sleeping)
        ...
        do {
            ...
            if (modeQueuePort != MACH_PORT_NULL && livePort == modeQueuePort) {
                ...
                while (_dispatch_runloop_root_queue_perform_4CF(theMode->_queue));
                if (theMode->_timerFired) {
                    // Leave livePort as the queue port, and service timers below
                    theMode->_timerFired = false;
                    break;
                } else {
        ...
        __CFRunLoopWaitForMultipleObjects(waitSet, NULL, poll ? 0 : TIMEOUT_INFINITY, theMode->_msgQMask, &livePort, &windowsMessageReceived);
#endif
        
        __CFRunLoopLock(runLoop);
        __CFRunLoopModeLock(theMode);

        runLoop->_sleepTime += (poll ? 0.0 : (CFAbsoluteTimeGetCurrent() - sleepStart));
        ...
        __CFRunLoopSetIgnoreWakeUps(runLoop);

        // user callouts now OK again
	__CFRunLoopUnsetSleeping(runLoop);
	if (!poll && (theMode->_observerMask & kCFRunLoopAfterWaiting)) __CFRunLoopDoObservers(runLoop, theMode, kCFRunLoopAfterWaiting);
	    ...
	    __CFRunLoopModeLock(theMode);
 	    sourceHandledThisLoop = true;
            ...
            __CFRunLoopSetSleeping(runLoop);
            __CFRunLoopModeUnlock(theMode);
            __CFRunLoopUnlock(runLoop);
        ...
        if (MACH_PORT_NULL == livePort) {
            ...
            // handle nothing
        } else if (livePort == runLoop->_wakeUpPort) {
            CFRUNLOOP_WAKEUP_FOR_WAKEUP();
            // do nothing on Mac OS
#if DEPLOYMENT_TARGET_WINDOWS
            // Always reset the wake up port, or risk spinning forever
            ResetEvent(runLoop->_wakeUpPort);
#endif
        }
#if USE_DISPATCH_SOURCE_FOR_TIMERS
        else if (modeQueuePort != MACH_PORT_NULL && livePort == modeQueuePort) {
            CFRUNLOOP_WAKEUP_FOR_TIMER();
            if (!__CFRunLoopDoTimers(runLoop, theMode, mach_absolute_time())) {
                // Re-arm the next timer, because we apparently fired early
                __CFArmNextTimerInMode(theMode, runLoop);
            }
        }
#endif
#if USE_MK_TIMER_TOO
        else if (theMode->_timerPort != MACH_PORT_NULL && livePort == theMode->_timerPort) {
            CFRUNLOOP_WAKEUP_FOR_TIMER();
            ...
            CFRunLoopSourceRef rls = __CFRunLoopModeFindSourceForMachPort(runLoop, theMode, livePort);
            if (rls) {
#if DEPLOYMENT_TARGET_MACOSX || DEPLOYMENT_TARGET_EMBEDDED || DEPLOYMENT_TARGET_EMBEDDED_MINI
		mach_msg_header_t *reply = NULL;
		sourceHandledThisLoop = __CFRunLoopDoSource1(runLoop, theMode, rls, msg, msg->msgh_size, &reply) || sourceHandledThisLoop;
		if (NULL != reply) {
		    (void)mach_msg(reply, MACH_SEND_MSG, reply->msgh_size, 0, MACH_PORT_NULL, 0, MACH_PORT_NULL);
		    CFAllocatorDeallocate(kCFAllocatorSystemDefault, reply);
		}
#elif DEPLOYMENT_TARGET_WINDOWS
                sourceHandledThisLoop = __CFRunLoopDoSource1(runLoop, theMode, rls) || sourceHandledThisLoop;
#endif
	    }
	...
	if (sourceHandledThisLoop && stopAfterHandle) {
        ...
        theMode->_stopped = false;
	    retVal = kCFRunLoopRunStopped;
	} else if (__CFRunLoopModeIsEmpty(runLoop, theMode, previousMode)) {
...
SInt32 CFRunLoopRunSpecific(CFRunLoopRef rl, CFStringRef modeName, CFTimeInterval seconds, Boolean returnAfterSourceHandled) {     /* DOES CALLOUT */
    ...
    CFRunLoopModeRef previousMode = rl->_currentMode; //取上一次运行的mode
...
/** 启动RunLoop
 * 直到超时或被手动停止，该函数才会返回。
 * 每次线程运行RunLoop都会自动处理之前未处理的消息，并且将消息发送给观察者，让事件得到执行。
 * RunLoop运行时首先根据modeName找到对应mode，如果mode里没有source/timer/observer，直接返回。
 */
/// 此处为程序启动时指定的 RunLoop 用 DefaultMode 启动
