...
/*
 ...
 1、下载（SDWebImageDownloader）
 2、缓存（SDImageCache）
 3、将缓存和下载的功能组合起来（SDWebImageManager），并与 UIKit 层进行交互
 4、封装成 UIImageView 等类的分类方法（UIImageView+WebCache 等），为表层的 UIKit 框架提供接口
 */

/* 图片下载 SDWebImageDownloader
 
 1、如何实现异步下载，也就是多张图片同时下载？
 2、如何处理同一张图片（同一个 URL）多次下载的情况？
 
 */

//下载选项：这些选项主要涉及到下载的优先级、缓存、后台任务执行、cookie处理以认证几个方面。
typedef NS_OPTIONS(NSUInteger, SDWebImageDownloaderOptions) {
    ...
    在iOS 4+系统上，允许程序进入后台后继续下载图片。该操作通过向系统申请额外的时间来完成后台下载。
     如果后台任务终止，则操作会被取消
     */
...
typedef NS_ENUM(NSInteger, SDWebImageDownloaderExecutionOrder) {
     ...
     * Default value. All download operations will execute in queue style (first-in-first-out).
      以队列的方式，按照 先进先出 的顺序下载。这是默认的下载顺序
     */
...
/*
 每一个图片的下载都会对应一些回调操作，如下载进度回调，下载完成回调等，这些回调操作是以block形式来呈现，
 为此在SDWebImageDownloader.h中定义了几个block，如下所示：
 */
//下载进度
