...
ALWAYS_INLINE Class & classForIndex(uintptr_t index) {
    assert(index > 0);
...
inline bool objc_object::isWeaklyReferenced(){
    assert(!isTaggedPointer());
...
ALWAYS_INLINE id objc_object::rootRetain(){// -retain 方法
...
inline void objc_object::initInstanceIsa(Class cls, bool){
    initIsa(cls);
}


inline void objc_object::initIsa(Class cls, bool, bool){
    initIsa(cls);
}
...
inline bool objc_object::rootReleaseShouldDealloc(){
    if (isTaggedPointer()) return false;
...
static ALWAYS_INLINE bool callerAcceptsOptimizedReturn(const void * const ra0){
...
static ALWAYS_INLINE bool prepareOptimizedReturn(ReturnDisposition disposition __unused){
    return false;
}
