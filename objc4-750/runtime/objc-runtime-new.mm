...
/* 向哈希表allocatedClasses中添加一个类；如果addMeta为真，也会自动添加类的元类。
 * Locking: runtimeLock must be held by the caller.
 */
...
static void addNamedClass(Class cls, const char *name, Class replacing = nil){
    ...
    if ((old = getClass(name))  &&  old != replacing) {
        inform_duplicate(name, old, cls);//警告重复的类实现
        // getNonMetaClass() 使用名称查找;没有通过名称查找找到的类插入哈希表 nonmeta_class_map
        addNonMetaClass(cls);
    } else {
...
static void addFutureNamedClass(const char *name, Class cls){
    ...
    rw->ro = ro;
    cls->setData(rw);
    cls->data()->flags = RO_FUTURE;//flags = RO_FUTURE 未实现的future class
...
/* 从哈希表 future_named_class_map 中移除指定的类
 * @param name 类名，该类已实现，需要从表中移除
 * @return 只有在哈希表 future_named_class_map 中存在 name 对应的 class，才会返回它；否则返回 NULL；
 */
...
/* 向重新映射类的哈希表中插入一组映射关系
 * newcls 是一个已实现的 future class ，它将取代旧的 cls。
 * 或者 newcls 为 nil ，替换被忽略的弱链接类 oldcls。
...
/* 如果某些 cls 的超类是弱链接的并且丢失了，则返回YES。
 */
static bool missingWeakSuperclass(Class cls){
    assert(!cls->isRealized());
    
    if (!cls->superclass) {
        // superclass nil. 这只适用于根类
        return (!(cls->data()->flags & RO_ROOT));
    } else {
        // superclass 非 nil ，检查是否缺少更高的 superclass
...
Class readClass(Class cls, bool headerIsBundle, bool headerIsPreoptimized){
    ...
    // 注意: 类 __ARCLite__ 的hack不能通过这里;适用于它的类结构修正也需要在下面的非延迟实现中执行。
    // 由于_objc_empty_vtable的绑定，这些字段应设置为0，但OS X 10.8的dyld不会按预期绑定共享缓存 absolute symbols。
    // This (and the __ARCLite__ hack below) can be removed once the simulator drops 10.8 support.
#if TARGET_OS_SIMULATOR
    ...
    /* 1、从哈希表 future_named_class_map 中获取 mangledName 对应的类，则并表中移除该键值对
     * 2、将旧类的 class_ro_t 信息赋值到新类上；
     * 3、旧类与新类映射到哈希表 remapped_class_map 中
     */
    Class replacing = nil;
    if (Class newCls = popFutureNamedClass(mangledName)) {
        // 这个名称以前被分配为 future class
        // 将 objc_class 复制到 future class 的结构中
        // 保存 future 的 rw 数据块
        
        if (newCls->isAnySwift()) {
            _objc_fatal("Can't complete future class request for '%s' because the real class is too big.",
                        cls->nameForLogging());
        }

        
        /* 从文件读取的类的结构与运行时类的结构是不同的
         * 类的结构 objc_class->ro
         * 运行时类的结构 objc_class->rw->ro
         * cls 是原始结构 ro 上存储着类的很多信息
         * newCls 是运行时创建的类，rw 与 rw->ro 仅仅在 addFutureNamedClass() 函数分配内存，但是并没有对应的信息
         * 下述代码：将cls上信息 ro 赋值到 newCls->rw->ro，然后还需要将 old_ro 释放
         */
        class_rw_t *rw = newCls->data();
        const class_ro_t *old_ro = rw->ro;
        //从 cls 所指的内存地址的起始位置开始拷贝 sizeof(objc_class)个字节到newCls所指的内存地址的起始位置中
    ...
    if (headerIsPreoptimized  &&  !replacing) {
        // class list built in shared cache fixme strict assert doesn't work because of duplicates assert(cls == getClass(name));
        assert(getClass(mangledName));
    } else {
        addNamedClass(cls, mangledName, replacing);//加入哈希表 gdb_objc_realized_classes ：mangledName=>cls
...
hIndex++
    ...
    /* 1、初始化 TaggedPointer混淆器、哈希表 gdb_objc_realized_classes 与 allocatedClasses */
    if (!doneOnce) {//保证只加载一次
        doneOnce = YES;
...
# if TARGET_OS_OSX
        ...
        if (dyld_get_program_sdk_version() < DYLD_MACOSX_VERSION_10_11) {
            DisableNonpointerIsa = true;
            if (PrintRawIsa) {
                _objc_inform("RAW ISA: disabling non-pointer isa because the app is too old (SDK version " SDK_FORMAT ")",FORMAT_SDK(dyld_get_program_sdk_version()));
            }
        }
...
# endif
        ...
        initializeTaggedPointerObfuscator();//初始化TaggedPointer混淆器
        ...
        gdb_objc_realized_classes = NXCreateMapTable(NXStrValueMapPrototype, namedClassesSize);//预优化的类不在该表
...
//    }
    ...
    if (PrintPreopt) {
        ...
        for (EACH_HEADER) {
            if (hi->isPreoptimized()) {
                _objc_inform("PREOPTIMIZATION: honoring preoptimized selectors in %s", hi->fname());
            }
            else if (hi->info()->optimizedByDyld()) {
                _objc_inform("PREOPTIMIZATION: IGNORING preoptimized selectors in %s", hi->fname());
            }
        ...
        _objc_inform("PREOPTIMIZATION: %zu selector references not pre-optimized", UnfixedSelectors);
...
#pragma mark - 协议 objc_property_t
 ...
 * Locking: acquires runtimeLock
 **********************************************************************/
Method protocol_getMethod(protocol_t *proto, SEL sel, bool isRequiredMethod, bool isInstanceMethod, bool recursive)
{
    if (!proto) return nil;
 ...
 * Locking: runtimeLock must be held by the caller
 **********************************************************************/
const char * protocol_getMethodTypeEncoding_nolock(protocol_t *proto, SEL sel,
                                      bool isRequiredMethod,bool isInstanceMethod){
    runtimeLock.assertLocked();
 ...
 * Locking: acquires runtimeLock
 **********************************************************************/
const char * _protocol_getMethodTypeEncoding(Protocol *proto_gen, SEL sel,
                                BOOL isRequiredMethod, BOOL isInstanceMethod){
 ...
 * Locking: none
 **********************************************************************/
const char * protocol_t::demangledName(){
 ...
 * Locking: runtimeLock must be held by the caller
 **********************************************************************/
static property_t * protocol_getProperty_nolock(protocol_t *proto, const char *name,
...
objc_property_t * protocol_copyPropertyList(Protocol *proto, unsigned int *outCount){
    return protocol_copyPropertyList2(proto, outCount, YES/*required*/, YES/*instance*/);
}
 ...
 * Locking: acquires runtimeLock
 **********************************************************************/
Protocol * __unsafe_unretained * protocol_copyProtocolList(Protocol *p, unsigned int *outCount){
    unsigned int count = 0;
    Protocol **result = nil;
    protocol_t *proto = newprotocol(p);
...
/* 创建一个协议对象；在调用 objc_registerProtocol() 之前，不能使用一个没有注册的协议；
 * 如果已经存在同名协议，则返回 nil
 */
...
/* 注册一个新构造的协议；注册之后该协议才可以使用，并且注册之后该协议不能改变；
 */
 ...
 * Locking: acquires runtimeLock
 **********************************************************************/
void protocol_addProtocol(Protocol *proto_gen, Protocol *addition_gen){
    protocol_t *proto = newprotocol(proto_gen);
 ...
 * Locking: acquires runtimeLock
 **********************************************************************/
static void protocol_addMethod_nolock(method_list_t*& list, SEL name, const char *types){
    if (!list) {
 ...
 * Locking: acquires runtimeLock
 **********************************************************************/
static void protocol_addProperty_nolock(property_list_t *&plist, const char *name,
 ...
 **********************************************************************/
static void saveTemporaryString(char *str){
...
const char *objc_class::demangledName(bool realize) {
    ...
    //重组符号名称
    const char *mangled = mangledName(); // class_ro_t->name;
...
static void log_and_fill_cache(Class cls, IMP imp, SEL sel, id receiver, Class implementer){
    ...
    if (objcMsgLogEnabled) {
        bool cacheIt = logMessageSend(implementer->isMetaClass(),cls->nameForLogging(),
                                      implementer->nameForLogging(),sel);
        if (!cacheIt) return;
    }
#endif
...
/* 在类中查找方法并加载缓存
 * 类似于 _class_lookupMethodAndLoadCache，但不搜索超类。
 * 如果在类中没有找到方法，则缓存并返回 objc_msgForward
 */
 ...
 * Locking: runtimeLock must be held by the caller
 **********************************************************************/
static SEL *addMethods(Class cls, const SEL *names, const IMP *imps, const char **types,
...
void class_replaceMethodsBulk(Class cls, const SEL *names, const IMP *imps,
...
void class_replaceProperty(Class cls, const char *name,const objc_property_attribute_t *attrs, unsigned int n){
 ...
 */
Class look_up_class(const char *name,bool includeUnconnected __attribute__((unused)),
 ...
 **********************************************************************/
bool verifySuperclass(Class superclass, bool rootOK){
    if (!superclass) {
...
void objc_registerClassPair(Class cls){
    ...
    {
        _objc_inform("objc_registerClassPair: class '%s' was already registered!", cls->data()->ro->name);
        return;
    }
    ...
    {
        _objc_inform("objc_registerClassPair: class '%s' was not allocated with objc_allocateClassPair!",
                     cls->data()->ro->name);
        return;
    }
...
#endif
unsigned class_createInstances(Class cls, size_t extraBytes,id *results, unsigned num_requested){
 ...
 * Locking: none
 **********************************************************************/
id object_copyFromZone(id oldObj, size_t extraBytes, void *zone){
 ...
 */
#if !SUPPORT_TAGGED_POINTERS //禁用 tagged pointer 对象

//这些变量总是为 debuggers 提供的。
...
#else //启用 tagged pointer 对象

// 类表中使用并提供给调试器的“slot”包括 tagged 的位;这比 objc_msgSend() 更快。
// "ext" 表示不这样做。
...
/* 在数组中获取指定索引的类指针；
 * @param tag 索引
 * @note 如果 0 <=tag < 7 ,则去数组 objc_tag_classes 中查找对应的类
 * @note 如果 8 <=tag < 264 ,则去数组 objc_tag_ext_classes 中查找对应的类
 */
...
void _objc_registerTaggedPointerClass(objc_tag_index_t tag, Class cls){
    if (objc_debug_taggedpointer_mask == 0) {// TaggedPointer 被禁用
        //在 TaggedPointer 被禁用时注册TaggedPointer 类是没有意义的，终止程序
        _objc_fatal("tagged pointers are disabled");//终止程序
    }
    
    //根据索引获取指定的类指针
    Class *slot = classSlotForTagIndex(tag);
    if (!slot) {
        //传递无效的索引时，无法正确获取指定的类指针
