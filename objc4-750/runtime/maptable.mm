...
/* 获取指定哈希表中的指定键，并将对应的值写入缓存 value
 * @param table 指定的哈希表
 * @param key 指定的键
 * @param value 要写入键值的内存地址
 * @return 如果该哈希表中存在键 key ，则返回哈希表中的键 key ；否则返回 NX_MAPNOTAKEY
 */
static INLINE void *_NXMapMember(NXMapTable *table, const void *key, void **value) {
    ...
    if (isEqual(table, pair->key, key)) {//判断指定 key 的 value 是否相等
        *value = (void *)pair->value;//将指针value赋值
 ...
 * @param table 指定的哈希表
 * @param key 指定的键
 * @return 返回键所对应的值；如果没有该键，则返回 NULL
 */
